#!/bin/bash
export LD_LIBRARY_PATH=~/.local/lib/fcitx5:$LD_LIBRARY_PATH
cat << 'RUST' > hc_core/tests_vni_workflow.rs
use hc_core::*;
...
cargo test --manifest-path hc_core/Cargo.toml --test tests_vni_workflow -- --nocapture
