...
static GLOBAL_MACROS: OnceLock<Arc<RwLock<HashMap<String, String>>>> = OnceLock::new();

pub(crate) fn get_global_macros() -> Arc<RwLock<HashMap<String, String>>> {
    GLOBAL_MACROS
...
pub struct CompositionEngine {
    ...
    pub macros: Arc<RwLock<HashMap<String, String>>>,
...
impl CompositionEngine {
    ...
    pub fn commit_current(&mut self) -> HC_State {
        ...
        let macro_match = self.macros.read().unwrap().get(&raw_lower).cloned();
...
pub fn vni_digit_transforms_buffer(buffer: &str, ch: char, legacy_tone: bool) -> bool {
    TypingEngine::vni_digit_transforms_buffer(buffer, ch, legacy_tone)
}
